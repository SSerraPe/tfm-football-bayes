...
> **Note (2026-09-07):** the dimensions and results below (P=53, N=4,944, 1,650 players)
> reflect the original pre-rebuild dataset these stages were first run against. Stage 03 was
> later refit on the rebuilt, GK-excluded dataset (P=48, N=4,586, I=1,529 — see CLAUDE.md §8,
> "Stage 03 and 10 refit outputs on rebuilt data"); this note's methodology and step-by-step
> description remain accurate, but the specific numbers in the table below are historical.

This note explains the modelling stages after the model objects were prepared. The aim of these stages is to move from a real-data Bayesian fit to simulation-based checks and final validation. The modelling unit is one observed player-season row, with 53 scaled football features, 4,944 rows, 1,650 players, and 12 seasons (the dimensions at the time this note was written — see the note above).
