...
The need for this background is already visible in the exploratory analysis conducted on the player--season dataset. The data is built from event-based football statistics provided by \emph{Wyscout}, a professional football data platform that collects and structures detailed match event information for scouting and performance analysis. From these event records, season-level aggregates are constructed and organised into the football blocks we defined previously. Once such multivariate profiles are defined, a number of statistical questions arise naturally: how many latent dimensions are needed to summarise the main structure, whether the variables reflect a smaller number of underlying stylistic components, and whether the resulting representation is stable and interpretable enough to support meaningful comparisons between players.
...
{\|\tilde{x}_i\|\,\|\tilde{x}_j\|}.
\]
Thus, correlation is the cosine similarity between centred profiles. This makes correlation relevant for the later PCA and factor-analytic modelling, since these methods are directly based on covariance/correlation structure.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.55\textwidth]{images/scatter_distances.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.55\textwidth]{images/scree_pca.png}
...
\small
\begin{tabular}{lr@{\hspace{1.3em}}lr@{\hspace{1.3em}}lr}
\toprule
\multicolumn{2}{c}{PC1} & \multicolumn{2}{c}{PC2} & \multicolumn{2}{c}{PC3} \\
\cmidrule(lr){1-2}\cmidrule(lr){3-4}\cmidrule(lr){5-6}
dribbles          &  0.318 & head shots       & -0.298 & defensive duels    & -0.342 \\
touch in box      &  0.251 & progressive run  &  0.295 & fouls              & -0.341 \\
shots             &  0.244 & accelerations    &  0.279 & successful dribbles&  0.303 \\
accelerations     &  0.229 & aerial duels     & -0.242 & loose ball duels   & -0.268 \\
progressive run   &  0.225 & dribbles         &  0.215 & dribbles against   & -0.263 \\
\midrule
\multicolumn{2}{c}{PC4} & \multicolumn{2}{c}{PC5} & & \\
\cmidrule(lr){1-2}\cmidrule(lr){3-4}
successful dribbles    &  0.308 & successful dribbles  &  0.754 & & \\
accelerations          & -0.299 & offensive duels won  &  0.281 & & \\
progressive run        & -0.286 & accelerations        &  0.217 & & \\
passes to final third  &  0.228 & progressive run      &  0.177 & & \\
through passes         &  0.223 & dribbles             &  0.171 & & \\
\bottomrule
\end{tabular}
\end{table}
...
More specifically, the root mean square residual (RMSR) summarises the average size of the residual correlations left unexplained by the fitted factor model, while the root mean square error of approximation (RMSEA) measures lack of fit per degree of freedom at the covariance-structure level \cite{bartholomew2011latent,browne1992fit}. The Tucker--Lewis index (TLI), also known as the non-normed fit index, compares the fitted model's discrepancy per degree of freedom against that of a baseline model in which all variables are uncorrelated, so that values closer to \(1\) indicate a comparatively better, more parsimony-adjusted fit \cite{tucker1973reliability}. Model comparison also considered the Bayesian information criterion (BIC), defined in the sense of Schwarz \cite{schwarz1978bic}, which balances model fit against complexity.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.55\textwidth]{images/scree_fa.png}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{images/loadings_fa.png}
    \caption{Rotated factor loadings for the eleven-factor oblimin solution, displaying variables with $|\mathrm{loading}|\geq 0.30$. Blue and red bars indicate positive and negative loadings, respectively. Panels are ordered by variance explained (F1 highest) and labeled with the corresponding interpretation, matching the enumeration above.}
...
\begin{figure}[h]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/cmds_cosine.png}
        \caption{Coloured by minutes played. The absence of a clear minutes gradient suggests that the embedding is not primarily driven by playing time.}
        \label{fig:cmds_cosine}
    \end{subfigure}\hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/cmds_extreme_obs.png}
        \caption{Extreme observations labelled. Dimension 1 is anchored by wide attacking profiles (positive) vs.\ centre-back profiles (negative); Dimension 2 contrasts aerial/box-reference forwards with attacking full-backs.}
        \label{fig:cmds_extremes}
    \end{subfigure}
    \caption{Two-dimensional CMDS embedding of the player--season observations using cosine-based dissimilarity.}
    \label{fig:cmds_combined}
\end{figure}
