...
\begin{figure}[h]
    ...
    \caption{Pairwise Euclidean vs. cosine dissimilarities for the player-season subset (N = 900, ~30,000 sampled pairs). The two metrics show moderate-to-strong agreement (Pearson = 0.744, Spearman = 0.764).}
...
\begin{figure}[h]
    ...
    \caption{Scree plot of the first 30 principal components. PC1 and PC2 jointly explain approximately \(52\%\) of the variance, while the first ten components explain approximately \(91\%\). The dashed line marks the first component count reaching at least \(90\%\) cumulative explained variance.}
    \label{fig:scree_plot}
\end{figure}

The exploratory results suggest that the data are structured but not trivially so. In particular, \autoref{fig:scree_plot} shows that the first two principal components explain \(51.8\%\) of the total sample variance, the first five around \(76.0\%\), and the first ten roughly \(90.9\%\). Using \(90\%\) cumulative explained variance as a descriptive threshold therefore suggests retaining about ten principal components for a high-fidelity linear summary. This indicates that a low-dimensional summary is plausible, but also that the player--season feature space cannot be reduced to a single simple axis without substantial information loss. To interpret the leading components beyond their explained variance, the variables with the largest absolute loadings were inspected. \autoref{tab:pca_loadings} reports the five largest absolute loadings for each of the first five principal components.
...
\midrule
PC1 & \texttt{dribbles\_p90}     &  0.394 \\
PC1 & \texttt{recov\_p90}        & -0.280 \\
PC1 & \texttt{prog\_passes\_p90} & -0.275 \\
PC1 & \texttt{aerial\_win}       & -0.255 \\
PC1 & \texttt{duels\_p90}        &  0.249 \\
\midrule
PC2 & \texttt{dribbles\_p90}     &  0.450 \\
PC2 & \texttt{prog\_pass\_acc}   &  0.384 \\
PC2 & \texttt{aerial\_p90}       & -0.379 \\
PC2 & \texttt{passes\_p90}       &  0.347 \\
PC2 & \texttt{prog\_passes\_p90} &  0.302 \\
\midrule
PC3 & \texttt{dribble\_succ}     & -0.786 \\
PC3 & \texttt{duel\_win}         & -0.302 \\
PC3 & \texttt{def\_actions\_p90} &  0.239 \\
PC3 & \texttt{xg\_p90}           & -0.202 \\
PC3 & \texttt{goals\_p90}        & -0.202 \\
\midrule
PC4 & \texttt{duels\_p90}        & -0.491 \\
PC4 & \texttt{aerial\_p90}       & -0.357 \\
PC4 & \texttt{dribble\_succ}     & -0.357 \\
PC4 & \texttt{pass\_acc}         &  0.336 \\
PC4 & \texttt{def\_actions\_p90} & -0.308 \\
\midrule
PC5 & \texttt{duel\_win}         &  0.486 \\
PC5 & \texttt{dribble\_succ}     & -0.406 \\
PC5 & \texttt{aerial\_p90}       &  0.383 \\
PC5 & \texttt{aerial\_win}       &  0.358 \\
PC5 & \texttt{prog\_pass\_acc}   &  0.268 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h]
    ...
    \caption{Parallel analysis scree plot for factor analysis. The observed eigenvalues are compared with the $95$th percentile of eigenvalues obtained from simulated random datasets. The vertical dashed line indicates the suggested number of retained factors, $k=6$, corresponding to the point after which the observed eigenvalues no longer exceed the simulated reference values.}
    \label{fig:parallel_analysis}
\end{figure}

The exploratory results support the presence of non-trivial latent structure. Parallel analysis suggested a six-factor solution, while a four-factor model provided a more compressed but clearly rougher summary. The six-factor specification improved the fit statistics relative to the four-factor one, with lower RMSEA and RMSR and a markedly improved BIC, although the overall fit still indicates that the latent structure should be viewed as an approximation rather than a complete explanation of the dependence in the data. Taken together, the pattern in \autoref{fig:parallel_analysis} and the fit comparisons suggest that six factors offer a reasonable compromise between parsimony and interpretability.

Substantively, the rotated loading structure reveals several interpretable latent dimensions; see \autoref{fig:fa_loadings}. In particular, the six-factor solution separates in the following dimensions: goal-threat, pass-circulation, defensive-work, duel-efficiency, physical-activity-oriented, and a mixed creativity-versus-aerial profile. While these labels are necessarily interpretive, the loading pattern shows that the latent dimensions are not arbitrary mathematical artefacts, but correspond to recognisable football behaviours. Importantly, the estimated factor correlations are not negligible, which reinforces the idea that football style may be better described through related latent traits than through a small set of orthogonal descriptive axes.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{images/loadings_fa.png}
    \caption{Rotated factor loadings for the six-factor oblimin solution, displaying variables with $|\mathrm{loading}|\geq 0.30$. Blue and red bars indicate positive and negative loadings, respectively. The loading pattern supports an interpretation of the factors in terms of goal threat, duel efficiency, physical activity, pass circulation, creativity versus aerial profile, and defensive work.}
...
\begin{figure}[h]
    ...
    \caption{CMDS map with extreme observations labelled. The four groups of extremes correspond to high and low values along each displayed dimension. Dimension 1 is mainly anchored by defensive profiles on the positive side and attacking profiles on the negative side, while Dimension 2 contrasts aerial or box-reference forwards with more technical circulation/progression profiles.}
    \label{fig:cmds_extremes}
\end{figure}

The labelled extremes in \autoref{fig:cmds_extremes} help interpret the two displayed dimensions. The positive side of Dimension 1 is anchored mainly by centre-back profiles, such as Jorge Cuenca, Fernando Calero, Yeray Álvarez, N. Araújo, and Alejandro Gálvez, whereas the negative side contains more attacking or forward-oriented profiles, such as Iñaki Williams, Oihan Sancet, José Luis Morales, Ángel Correa, and Álex Berenguer. Dimension 2 separates aerial or penalty-box reference forwards, such as S. Rondón, Hugo Duro, Y. En-Nesyri, Juanmi Latasa, and V. Muriqi, from more technical circulation or progression profiles, such as É. Banega, André Almeida, Marcelo, Filipe Luís, and M. Kovačić. These labels should be interpreted descriptively: they identify the observed profiles anchoring the CMDS axes, not fixed universal football roles.  For these reasons, CMDS is best viewed here as an exploratory geometric benchmark rather than as the final model. It indicates whether the chosen dissimilarity admits a coherent low-dimensional approximation, and it provides a descriptive precursor to more structured probabilistic latent-space models, including Bayesian and hierarchical extensions \cite{oh2001bayesian,yanchenko2020hierarchical}.
