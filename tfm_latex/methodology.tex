...
This covariance structure is motivated directly by the exploratory findings of Chapter~\ref{chap:background}. Factor analysis of the player--season feature matrix suggested that a number of correlated latent traits---eleven, by parallel analysis---are needed to reproduce the empirical correlation structure of the raw, unpartitioned observations. A low-rank $\boldsymbol{\Sigma}_a$ directly encodes this qualitative finding (shared covariance is real and worth modelling with a factor structure) within a fully probabilistic model \cite{lopes2004bayesian}; the rank $K$ actually adopted for $\boldsymbol{\Sigma}_a$ is selected separately in Section~\ref{sec:rank}, and Remark~\ref{rem:k_not_comparable} explains why it need not, and does not, match the raw factor count above.
...
\label{eq:total_elpd}
\end{equation}
A natural \emph{reversal criterion} would select the smallest $K$ such that $\mathrm{ELPD}_{K+1} < \mathrm{ELPD}_K$: the first rank at which adding a further latent dimension no longer improves out-of-sample predictive performance.
\end{definition}

The hold-out approach is preferred here to $K$-fold cross-validation on computational grounds: fitting the full hierarchical model requires several hours per run, so $K_{\mathrm{fold}}$-fold cross-validation, which requires $K_{\mathrm{fold}}$ complete re-fits, would be prohibitive. Pareto-smoothed importance-sampling leave-one-out cross-validation (PSIS-LOO) \cite{vehtari2017practical} was also computed post hoc on full-sample fits at each candidate $K$, as a check that does not require a held-out split; a fast Pathfinder-based variational screen \cite{zhang2022pathfinder} was additionally run across a wider range of candidate ranks to explore the rank axis more cheaply than repeated full NUTS fits allow.

\begin{remark}[Why this screen does not, by itself, decide $K$]
\label{rem:cv_inconclusive}
Two practical problems undermine the reversal criterion as a stand-alone decision rule here. First, PSIS-LOO's reliability depends on the Pareto-$\hat{k}$ diagnostic for each observation, and in this application $\hat{k} > 0.5$ for the great majority of observations---a consequence of the hierarchical structure and the limited number of seasons per player---so the importance-weighting approximation is only directionally informative, not a precise estimator of $\mathrm{ELPD}_K$. Second, and more importantly, neither the player-holdout ELPD (evaluated for $K = 1, \ldots, 6$ on a $15\%$ player-level holdout) nor the wider Pathfinder screen (evaluated for $K = 0, \ldots, 8$) produces a clean reversal: the player-holdout ELPD is maximised at $K = 6$, with $K = 3$ and $K = 4$ both competitive and separated from the maximum by an amount comparable to the run-to-run variability of the approximation, while the Pathfinder screen peaks at $K = 5$ with $K = 3$ a local dip rather than a clear optimum. Read honestly, both screens are best summarised as \emph{flat across $K \in \{2, \ldots, 6\}$} rather than as pointing to a single preferred rank. Reporting a specific $K^*$ from either screen alone would overstate the precision these approximate, high-variance predictive estimates actually offer. The cross-validation evidence is retained here as a supplementary, non-deciding check---by contrast, the comparison of $K=0$ (diagonal) against $K \geq 1$ (low-rank) is decisive by the same ELPD-based reasoning, and is reported in Chapter~\ref{chap:results}---rather than as the basis for choosing among $K \geq 1$.
\end{remark}

\subsection{A qualitative argument for the rank}
\label{sec:rank_qualitative}

Because the cross-validation screen of \S\ref{sec:holdout} does not discriminate among the candidate ranks $K \in \{2, \ldots, 6\}$, the rank actually adopted for the production model is selected on qualitative, structural grounds instead: the adequacy of each fit's own residuals, and the extent to which the shared player-effect covariance $\boldsymbol{\Sigma}_a$ is still adding well-identified structure as $K$ increases. This mirrors standard practice in exploratory and confirmatory factor analysis, where the number of retained factors is routinely set by fit diagnostics and interpretability rather than by a single automated rule, and is consistent with the professional statistical guidance received during supervision of this thesis: that a qualitative argument, grounded in residuals and in the variance structure of $\boldsymbol{\Lambda}\boldsymbol{\Lambda}^\top$, is an appropriate and sufficient basis for closing the choice of $K$.

\begin{remark}[Anchoring to the posterior mean, not to per-draw estimates]
\label{rem:posterior_mean_anchor}
Both criteria below require the eigendecomposition of the common-variance matrix $\boldsymbol{\Lambda}\boldsymbol{\Lambda}^\top$ introduced in Section~\ref{sec:pca_postprocessing}. Proposition~\ref{prop:spectral_inv} shows that, \emph{for a fixed posterior draw}, this eigendecomposition is invariant to the identification constraint used to fit $\boldsymbol{\Lambda}$. It is not, however, invariant \emph{across} posterior draws in the sense needed to build a credible interval directly: sampling variability in $\boldsymbol{\Lambda}^{(s)}$ can rotate the leading eigenvectors within the subspace they span from one draw to the next, so naively comparing $\mathbf{V}^{(s)}$ across draws risks comparing different rotations of the same subspace rather than a stable estimate of one direction. To avoid this, the analyses below fix a single reference orientation---the eigendecomposition of the \emph{posterior-mean} common-variance matrix $\bar{\boldsymbol{\Lambda}}\bar{\boldsymbol{\Lambda}}^\top$, where $\bar{\boldsymbol{\Lambda}} = \tfrac{1}{S_{\mathrm{mc}}}\sum_s \boldsymbol{\Lambda}^{(s)}$---and project each draw's loadings onto this fixed reference before summarising uncertainty. This is a first-order (mean-rotation) approximation to a fully Procrustes-aligned per-draw analysis; it is adequate for the two qualitative judgements made here (whether a component's common-variance share is small, and whether its loadings are reliably non-zero), since both depend on the sign and approximate magnitude of the projected loadings rather than on precise per-draw geometry.
\end{remark}
...
\item[Criterion B: reliable-loader count under the fixed-reference PCA.] For the last canonical factor at rank $K$, the number of features whose $90\%$ credible interval (computed under Remark~\ref{rem:posterior_mean_anchor}'s fixed reference) excludes zero indicates whether that dimension is a well-identified axis with broad support across features, or a weakly identified direction loaded on by only a handful of features.
\item[Criterion C: residual adequacy under the fit's own $t(\hat\nu)$ noise model.] For each candidate fit, the standardised residuals $z_{np} = (y_{np} - \hat a_{i[n],p} - \hat b_{j[n],p})/(\hat\sigma_{e,p}\sqrt{\hat\nu/(\hat\nu-2)})$ are compared against the $99$th-percentile threshold $\tau_{99}$ implied by $t(\hat\nu)$. If the empirical exceedance rate across all $N \times P$ cells is at or below the nominal $1\%$, the residual distribution is adequate under the fitted noise model; this is a floor check on model misspecification, not a criterion that discriminates between two ranks that both pass it.
\end{description}

\subsection{Selected rank}
\label{sec:rank_selection}

Applying Criteria A--C across $K=2, 3, 4$ gives the following picture. The last canonical factor's common-variance share declines from $30.8\%$ at $K=2$ to $10.5\%$ at $K=3$ to $7.1\%$ at $K=4$---a continued but decelerating decline, not a sharp drop to a noise floor at any point (Criterion A). The reliable-loader count of the last factor is $45/48$ at $K=2$, $43/48$ at $K=3$, and $45/48$ at $K=4$: it does not fall monotonically, so the fourth canonical factor is, if anything, as reliably identified by this criterion as the third (Criterion B). Mean residual exceedance under $\tau_{99}$ is $0.74\%$, $0.77\%$, and $0.79\%$ at $K=2,3,4$ respectively---all comfortably below the $1\%$ floor and essentially flat, so residual adequacy holds at every rank examined and does not discriminate among them (Criterion C).

Taken together, this comparison does not produce a clean reversal at $K=4$ the way it does at $K=3$ relative to $K=2$: Criterion A mildly favours stopping at $K=3$ (the fourth factor's contribution, while still non-trivial, is smaller than the third's), while Criterion B gives no reason to prefer $K=3$ over $K=4$, and Criterion C is uninformative at this rank range. Read honestly, the qualitative evidence for $K=3$ over $K=4$ is a preference on parsimony grounds under a still-declining but not-yet-negligible marginal contribution, rather than a decisive structural verdict of the kind that favours $K\geq 1$ over $K=0$ or, more mildly, $K=3$ over $K=2$. Combined with the cross-validation screens of Remark~\ref{rem:cv_inconclusive}, where $K=4$ is likewise never clearly dominated, the honest summary is that the data do not sharply distinguish $K=3$ from $K=4$. $K=3$ is nonetheless retained as the production rank $K^*=3$, used throughout the remainder of this chapter and in Chapter~\ref{chap:results}: it is the more parsimonious of two ranks that the evidence does not clearly separate, and closing the model at a defensible, adequately-supported rank was judged preferable to an open-ended search for further, likely similarly ambiguous, comparisons at $K \geq 5$.

\begin{remark}[Comparability with the exploratory factor count of Chapter~\ref{chap:background}]
\label{rem:k_not_comparable}
Chapter~\ref{chap:background} reports an eleven-factor solution from classical, maximum-likelihood factor analysis of the raw player--season feature matrix. It is natural to ask why the Bayesian model's rank, $K^*=3$, is so much smaller. The two quantities are not estimates of the same object. The exploratory factor analysis of Chapter~\ref{chap:background} is fit directly to the raw player--season observations $\mathbf{y}_n$, treated as exchangeable rows with no player/season structure: its factor count therefore describes the rank of the \emph{total, unpartitioned} covariance of $\mathbf{y}_n$, in which any two features that co-vary for any reason---shared stable player skill, a league-wide tactical shift common to a subset of seasons, or coincidental season-to-season drift---contribute jointly to the recovered factor structure. The additive decomposition of Definition~\ref{def:additive}, by contrast, first partitions this total covariance into player ($\boldsymbol{\Sigma}_a$), season ($\boldsymbol{\Sigma}_b$), and residual ($\boldsymbol{\Sigma}_e$) components, and the rank $K$ studied in this section governs only $\boldsymbol{\Sigma}_a$: the covariance of what is stable about a player once season-level drift and observation-level noise have already been subtracted out. Since $\boldsymbol{\Sigma}_b$ is diagonal by construction (\S\ref{sec:covariance}, justified there on identifiability grounds given only $S=12$ seasons), any genuine cross-feature covariance that is specifically season-driven cannot be absorbed by $\boldsymbol{\Sigma}_b$ and is instead either left in the residual or folded into $\boldsymbol{\Sigma}_a$'s estimate; either way, $K$ is not required to reproduce the rank of a covariance matrix that additionally contains season-level and idiosyncratic sources of dependence. A smaller $K$ for the player-only covariance than the raw factor count is therefore the expected outcome of conditioning on more structure, not evidence against either analysis. This reasoning is offered as an explanation for the direction of the discrepancy; it has not been separately validated (for instance, by fitting a raw factor analysis restricted to a player-only, season-averaged version of the data to check whether its factor count moves closer to three), and should be read as such.
\end{remark}
