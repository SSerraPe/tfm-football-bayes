...
The residuals $\boldsymbol{\varepsilon}_n$ are assigned a diagonal covariance $\boldsymbol{\Sigma}_e = \mathrm{diag}(\sigma_{e,1}^2, \ldots, \sigma_{e,P}^2)$, reflecting the assumption that, conditional on the player and season effects, the $P$ features are uncorrelated at the observation level.
...
Failing to account for this heteroscedasticity may cause the model to attribute the elevated variance of low-minute observations to heavy residual tails (via a smaller $\nu$), which would represent misspecification. A natural extension introduces a scaling of the residual variance on an observational level.
...
The joint prior is the product of all terms in Definitions~\ref{def:prior_lambda}--\ref{def:prior_phi}, since all parameters are treated as independent then the joint prior density is:
\begin{align}
...
Several out-of-sample criteria were tried to settle $K$ directly, before the qualitative argument below was adopted instead. A hold-out expected log predictive density (ELPD) was computed on a season-stratified $15\%$ test split; Pareto-smoothed importance-sampling leave-one-out cross-validation (PSIS-LOO) \cite{vehtari2017practical} was computed post hoc on full-sample fits, though its Pareto-$\hat{k}$ diagnostic exceeded $0.5$ for most observations here, limiting it to a directional check rather than a precise one; and a faster Pathfinder-based variational screen \cite{zhang2022pathfinder} was run across a wider range of candidate ranks than repeated full NUTS fits would allow. None of these gave a clean answer among $K \geq 1$: the hold-out ELPD was maximised at $K=6$, with $K=3$ and $K=4$ competitive and separated from it by no more than run-to-run variability, while the Pathfinder screen peaked at $K=5$ with $K=3$ a local dip rather than a clear optimum --- both are best read as flat across $K \in \{2,\ldots,6\}$ rather than as pointing to one preferred rank. The one comparison these criteria did settle clearly is $K=0$ (diagonal) against any $K \geq 1$ (low-rank), which is decisive and is reported in Chapter~\ref{chap:results}. For choosing among $K \geq 1$, however, the resolution was purely qualitative, as described next.

\subsection{A qualitative argument for the rank}
\label{sec:rank_qualitative}

One caveat is worth flagging briefly before making that argument: the rank $K^*=3$ selected here is not expected to match the eleven-factor count found in Chapter~\ref{chap:background}'s exploratory analysis. The two describe different things, for reasons given in full in Remark~\ref{rem:k_not_comparable} below.
