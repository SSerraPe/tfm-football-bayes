...
Applying Criteria A--C to the $K=2$ and $K=3$ production fits gives: the last canonical factor's common-variance share drops from $30.8\%$ at $K=2$ to $10.5\%$ at $K=3$---still a substantive contribution, not yet at the noise floor---while its reliable-loader count falls only modestly, from $45/48$ to $43/48$ features, indicating a well-identified third axis rather than a spurious one (Criteria A and B); mean residual exceedance under $\tau_{99}$ is $0.74\%$ at $K=2$ and $0.77\%$ at $K=3$, both comfortably below the $1\%$ floor, so residual adequacy holds at both ranks and does not itself discriminate between them (Criterion C). Taken together, $K=3$ is preferred over $K=2$ on structural grounds (A, B) while matching it on residual adequacy (C), and is adopted as the production rank $K^*=3$ used throughout the remainder of this chapter and in Chapter~\ref{chap:results}. Rank $K=4$ is not yet compared on Criteria A and B against $K=2$/$K=3$ at the time of writing and is therefore not asserted to be dominated or preferred; the reversal noted in Remark~\ref{rem:cv_inconclusive}'s cross-validation screens (where $K=4$ is occasionally competitive with $K=3$) means this comparison is left as an open extension rather than settled here.
